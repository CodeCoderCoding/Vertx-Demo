...
public class AsyncResultExample {
    ...
    public static void runExample(Vertx vertx) {
        ...
        performAsyncOperation(true, ar -> {
            handleAsyncResult(ar);
        });
        
        performAsyncOperation(false, ar -> {
            handleAsyncResult(ar);
        });
    ...
    private static void performAsyncOperation(boolean success, Handler<AsyncResult<String>> handler) {
